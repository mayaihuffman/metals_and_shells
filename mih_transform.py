...
def create_arbPDB_from_xyz(coords, file_name='', atom_type='C', res_name='LIG', chain_id='A'): #was rep_coords

    preffix = "HETATM    1  {0}   {1} {2}   1    ".format(atom_type[0], res_name[:3], chain_id[0])
    suffix  = "  1.00  0.00           Z\n"
    out_pdb = []
...
class Transform(object):
    ...
    def calculate_symmetric_transform(self):
        ...
        for chain, xyz_total in xyz_total_by_chain.iteritems():
            ...
            final_33 = get_geo_center(xyz_total[len(xyz_total)/3:len(xyz_total)*2/3])
